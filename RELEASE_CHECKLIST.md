...
The September 1 local candidate fixes that catalog instability by registering the 10 browser tools once per page lifetime. Each persistent handler validates the current mission state before execution. A focused regression check prevents state mutations from aborting and recreating the catalog. This fix has passed the local automated suite but has not been deployed or repeated in the native browser acceptance run.
